 ...
 * @LastEditTime: 2024-01-30 17:49:52
  ...
  <div class="home_page mid">
    <div class="page_left center">
      ...
      <div class="wind_speed_num">
        ...
        <div class="wind_speed_str">{{ windSpeedArr[devData.speed] }}</div>
      ...
      <div class="wind_area">
        ...
        <div class="grid_area">
          <div style="transform: rotateX(60deg);">
            <div :style="computeXY(item)" v-for="item in devData.data_array" :key="item">
              <img src="@img/ic_body.png" />
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="page_right center">
      ...
      <div class="heat_num_area">
        ...
        <div class="heat_num_str">0{{ devData.id_num !== 0 ? devData.id_num : '' }}</div>
...
const excutePlayer = (playObj, item) => {
  ...
  if (devData.value.swing_mode != 0) {
    ...
    range[item.swing_leaf].location = item.wind_area * 14
    range[item.swing_leaf].length = item.isAll ? 49 : 15
...
let devData = ref({
  ...
  "data_array": [
    ...
    //   // 只有扫风才有用，其他时候用人的角度，如果是风逆人动，角度需要转化
    //   "wind_area": 2,  //风向（区域），0：左，1：中，2：右
    // },
    {
      "id": 1,
      "angel": 110,   //角度，50-130°
...
const startPlay = () => {
  // 3个人
  const windArea = [...new Set(devData.value.data_array.map(el => el.wind_area))];
  if (windArea.length != 2) {
    // 在同一区域或者三个区域 采用第一个人的数据
    sgvaObj.forEach((el, i) => {
      sgvaObj[i] = { ...el, ...devData.value.data_array[0], isAll: windArea.length == 3 }
    })
  } else {
    // 两个区域
    const minArea = Math.min(...windArea)
    devData.value.data_array.forEach(el => {
      if (el.wind_area == minArea) {
        console.log(el, '上摆风');
        sgvaObj[0] = { ...sgvaObj[0], ...el }
      } else {
        sgvaObj[1] = { ...sgvaObj[1], ...el }
      }
    })
  }
  playSvga()
...
watch(() => devData.value.data_array, (newValue, oldVaule) => {
  clearPlay();
  // 先清空画布然后再重新渲染 
  newValue.length && setTimeout(() => { startPlay() }, 500);
})
const computeXY = (item) => {
  ...
  */
  ...
  if (item.distance >= 400) {
    ...
    num = 1.75 - (100 - item.distance) * 0.0009
